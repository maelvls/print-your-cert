...
func signGuestbookPage(remoteRoots *x509.CertPool, kclient kubernetes.Interface, namespace string) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != "POST" {
			http.Error(w, fmt.Sprintf("Only the GET method is supported supported on the path %s.\n", r.URL.Path), http.StatusMethodNotAllowed)
			return
		...
		guestbookURL := "https://guestbook.print-your-cert.cert-manager.io:9090/write"
